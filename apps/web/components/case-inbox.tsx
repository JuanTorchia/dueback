...
export function CaseInbox() {
  ...
  const load = useCallback(async () => {
    ...
    } catch { setError("We could not refresh your follow-ups. Your last saved cases have not been changed."); }
  }, []);
  ...
  return <div className="case-inbox">
    {error ? <section className="card error" role="alert"><p>{error}</p><button type="button" onClick={() => void load()}>Try again</button></section> : null}
    {items?.length === 0 ? <section className="card empty-state"><h2>Nothing to chase yet</h2><p>Add a company promise and DueBack will keep it open until there is proof.</p><a className="button-link" href="/intake">Add a promise</a></section> : null}
