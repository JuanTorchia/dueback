...
export function CaseResult({ caseId }: { readonly caseId: string }) {
  ...
  return <div className="result-grid consumer-case-detail">
    ...
    <section className={`card outcome ${detail.outcome.accepted ? "verified" : "waiting"}`}><div className="eyebrow">{detail.outcome.accepted ? tr("Evidence accepted", "Evidencia aceptada", "Evidência aceita") : tr("Still working", "Sigue trabajando", "Ainda trabalhando")}</div><h2>{detail.outcome.title}</h2><p>{detail.outcome.explanation}</p><div className="claim-limit">{detail.outcome.limitation}</div></section>
