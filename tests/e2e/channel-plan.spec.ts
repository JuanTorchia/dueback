...
test.describe("channel plan authorization", () => {
  ...
  test("lets a person edit Gemini's certain fields and creates a new contract version", async ({ page }) => {
    let draft = makeDraftCase();
    await page.route("**/api/channels", (route) => route.fulfill({
      status: 200, contentType: "application/json", body: JSON.stringify([{
        channelType: "CONTROLLED_SANDBOX", status: "AVAILABLE", canSend: true, canReceive: true,
        supportsThreading: false, supportsDeliveryReceipt: true, supportsAuthenticatedReply: true,
        requiresUserOAuth: false, reasonCodes: [], checkedAt: "2026-08-17T12:00:00.000Z"
      }])
    }));
    await page.route("**/api/cases/case_12345678/plan", async (route) => {
      if (route.request().method() === "GET") {
      ...
      draft = {
        ...
        promiseDraft: {
          ...
          transactionRef: { ...draft.promiseDraft.transactionRef, value: command.revision.transactionRef }
        },
